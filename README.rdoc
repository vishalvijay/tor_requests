...
2 - Usage
  ...
  res = Tor::HTTP.post('posttestserver.com', {post_options:{"var" => "variable"}}, '/post.php?dir=example', 80)
  p res.code
  p res.body
...
You can set additional custom header fields for each request
  Tor::HTTP.get("google.com", {headers: {"Content-Type" => "application/json"}}, "/", 80)
  Tor::HTTP.get(URI('http://google.com/'), {headers: {"Content-Type" => "application/json"}})
